        ...
        <div className="space-y-1 pb-6">
          {searchQuery.trim() ? (
            <div className="animate-in fade-in slide-in-from-top-1 duration-300">
              <h2 className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] mb-3 px-3">
                Global Discovery
              </h2>
              {searching ? (
                ...
                searchResults.map((u) => (
                  ...
                  >
                    ...
                    <ChevronRight
                      size={14}
                      className="text-muted-foreground/30 opacity-0 group-hover:opacity-100 transition-all -translate-x-2 group-hover:translate-x-0"
                    />
                  </button>
                ))
              )}
              ...
              {chatsLoading && chats.length === 0 ? (
                <div className="space-y-2 px-1">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div
                      key={i}
                      className="h-20 bg-white/2 rounded-2xl animate-pulse"
                    />
                  ))}
                </div>
              ) : (
                    ...
                    >
                      <div className="relative">
                        <Avatar className="h-14 w-14 rounded-2xl border border-white/5 shadow-xl">
                          <AvatarImage
                            src={getAvatar(chat.participants.avatarUrl)}
                            className="object-cover"
                          />
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        <div className="flex justify-between items-center mb-1">
                          <p
                            className={`font-bold truncate text-[15px] ${isActive ? "text-primary" : "text-white group-hover:text-primary transition-colors"}`}
                          >
